...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-4 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ${isScrolled
      ...
      <div className={`transition-all duration-500 ${isScrolled ? 'px-4 py-2' : 'px-6 py-3'
        }`}>
        <div className="flex items-center justify-between">
              ...
              {isScrolled && (
                <div className="w-8 h-8  rounded-lg flex items-center justify-center">
                  <span className="text-white font-bold text-sm">
                    <img src='logo.jpg' alt='logo' className='w-10 h-auto' />
                  </span>
                </div>
              )}
              {!isScrolled && (
                <span className="text-2xl font-md text-white">CommandoAI</span>
              )}
            </div>
          </div>
          ...
          {!isScrolled && (
            <div className="hidden md:flex items-center space-x-8">
              ...
              <a href="#education" className="text-gray-200 p-1 transition-colors hover:rounded-md hover:p-1 hover:bg-gray-500/20">Education</a>
          ...
          <div className="hidden md:block">
            {isScrolled ? (
              <button className="ml-4 w-8 h-8 bg-gradient-to-r from-[#1e3a8a] to-[#1d4ed8] rounded-lg flex items-center justify-center hover:shadow-lg hover:shadow-blue-500/25 transition-all duration-300">
                <ArrowRight className="w-4 h-4 text-white" />
              </button>
            ) : (
              <button className="bg-gradient-to-r from-[#1e3a8a] to-[#1d4ed8] text-white px-6 py-2 rounded-full hover:shadow-lg hover:shadow-blue-500/25 transition-all">
                Get Started
              </button>
            )}
          </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden py-4">
            <div className="flex flex-col space-y-4">
              ...
              <button className="bg-gradient-to-r from-[#1e3a8a] to-[#1d4ed8] text-white px-6 py-2 rounded-full hover:shadow-lg hover:shadow-blue-500/25 transition-all w-full">
                Get Started
              </button>
            </div>
          </div>
